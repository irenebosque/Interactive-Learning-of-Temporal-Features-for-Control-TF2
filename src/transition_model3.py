...
class TransitionModel:
    ...
    def _refresh_image_plots(self, neural_network):
        ...
        if (self.t_counter + 2) % 4 == 0 and self.show_ae_output:
            ...
            '''
            ae_model_output = neural_network(self.network_input[-1])  # irene
    ...
    def _train_model_from_database(self, neural_network, database):
        ...
        for i in range(self.number_training_iterations):  # Train
            ...
            neural_network.sess.run(neural_network.train_transition_model,
                                    feed_dict={'transition_model/transition_model_input:0': np.reshape(predictions, [self.transition_model_sampling_size, self.image_width, self.image_width, 1]),
            ...
            '''
            neural_network.compile(optimizer='adam', loss=tf.keras.losses.MeanSquaredError())  # irene
    ...
    def get_state_representation(self, neural_network, observation):
        ...
        # state_representation = neural_network.compile(optimizer='adam',
        #        loss=tf.keras.losses.MeanSquaredError()) #irene

        state_representation = neural_network(self.network_input[-1])  # irene
