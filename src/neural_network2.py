...
class NeuralNetwork:
    ...
    def MyModel(self):
        ...
        action_in = tf.keras.layers.Input(shape=self.action_shape)
        #action_in = tf.keras.layers.Input(shape=(100))
        computed_lstm_hs = tf.keras.layers.Input(shape= self.lstm_hidden_state_shape)
        computed_lstm_hs = tf.keras.layers.Input(shape=(300), batch_size=1)
       ...
       # lstm_hidden_state = tf.cond(self.lstm_hs_is_computed, lambda:lstm_hidden_state_out , lambda: computed_lstm_hs)
        ...
        # Model creation
        model_transition_model_output = tf.keras.Model(inputs = [transition_model_input, action_in, computed_lstm_hs], outputs = transition_model_output)
